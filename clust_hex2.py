...
list_attributes = [unique_tod,unique_day,unique_month]
...
for i1 in range(nclusters):
    ...
    for i2 in range(len(list_all_combs)):
        subset_data = data_clust.loc[(data_clust['Disc TOD']==list_all_combs[i2][0]) & (data_clust['Day']==list_all_combs[i2][1])& (data_clust['Month']==list_all_combs[i2][2])]
    ...
    dict_cluster_hexprob_noweather[i1+1] = value_dict

## ================================================
# Save as pickle

with open("dict_hexdata_prob_noweather.pickle","wb") as freq1:
    pickle.dump(dict_cluster_hexprob_noweather, freq1)